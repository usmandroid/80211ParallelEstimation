...
int main(int argc, char *argv[]) {
	...
	char *const parmList_Seq[] = {"/opt/ibm/platform_mpi/bin/mpiCC", "-w", "-Wno-deprecated", "-o", "main", "main.c", "utils.o",NULL};
	// char *const parmList_OpenMP[] = {"/usr/bin/g++", "-fopenmp", "-w", "-o", "main_openmp", "main_openmp.c", "utils.o",NULL};
	char *const parmList_OpenMP[] = {"/opt/ibm/platform_mpi/bin/mpiCC", "-fopenmp", "-w", "-o", "main_openmp", "main_openmp.c", "utils.o",NULL};
	...
	if(pid == 0){
		...
		if(!strcmp(argv[1],"sequential")) {
			...
			execv("/opt/ibm/platform_mpi/bin/mpiCC", parmList_Seq);
		}
		if(!strcmp(argv[1],"openmp")){
			...
			execv("/opt/ibm/platform_mpi/bin/mpiCC", parmList_OpenMP);
		}
